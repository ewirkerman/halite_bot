	...
	while curr_wave and limit:
		...
		for loc in curr_wave:
			...
			if limit and (timer() - gameMap.clock) > time_limit:
				logger.error("Aborting turn due to time limit! curr_time = %s" % (timer()-gameMap.clock))
				return results
		curr_wave = next_wave
	return results
...
def take_turn(gameMap):
	...
	# l = list(all_capped_claims)
	
	# while all([balance.claim_complete_conditions for root in l]):
	# for claim in 
...
def main():
	...
	gameMap.oldGameMap = oldGameMap or gameMap
	clock = timer()
	gameMap.clock = clock
